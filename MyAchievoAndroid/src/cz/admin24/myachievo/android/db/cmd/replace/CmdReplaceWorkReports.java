...
public class CmdReplaceWorkReports extends CmdReplace {
    private static final String          SQL_INSERT = String.format("INSERT INTO %s ( %s ,%s , %s , %s , %s , %s , %s , %s  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                                                            WorkReportTable.TABLE_NAME, WorkReportTable.COLUMN_NAME_ID, WorkReportTable.COLUMN_NAME_PROJECT, WorkReportTable.COLUMN_NAME_PHASE,
    ...
    public void execute(SQLiteDatabase db) {
        ...
        try {
            for (WorkReport r : reports) {
                ...
                bind(statement, 2, r.getProject());
                bind(statement, 3, r.getPhase());
                bind(statement, 4, r.getActivity());
                bind(statement, 5, r.getRemark());
                bind(statement, 6, r.getDate());
                bind(statement, 7, r.getHours());
                bind(statement, 8, r.getMinutes());
